...
const benefitIcons = [Leaf, Sparkles, Activity, Heart, Wind, Brain];

export default function TaichiPage() {
  const t = useTranslations('pages.taichi');
  const tCommon = useTranslations('common');

  const benefits = t.raw('benefits.items') as Array<{ title: string; description: string }>;
  const therapeuticItems = t.raw('therapeutic.items') as string[];
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <motion.div
              ...
              className="text-center mb-12"
            >
              <Heart className="w-12 h-12 text-crimson mx-auto mb-6" />
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                {t('therapeutic.title')}
              </h2>
              <p className="text-muted-foreground text-lg">
                {t('therapeutic.description')}
              </p>
            </motion.div>
              ...
              <Card className="bg-card/50 border-crimson/20">
                <CardContent className="p-8">
                  <ul className="grid md:grid-cols-2 gap-4">
                    {therapeuticItems.map((item, index) => (
                      <motion.li
                        key={index}
      ...
      {/* Who Is It For */}
      <section className="py-16 md:py-24 bg-gradient-to-b from-transparent via-green-500/5 to-transparent">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
