...
const developmentIcons = [Target, Brain, Sparkles, Scale, Award, Heart];

export default function ChildrenPage() {
  const t = useTranslations('pages.children');
  const tCommon = useTranslations('common');

  const methods = t.raw('approach.methods') as string[];
  const development = t.raw('development.items') as Array<{ title: string; description: string }>;
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {development.map((item, index) => {
              const Icon = developmentIcons[index];
              return (
                <motion.div
                  key={item.title}
                  initial={{ opacity: 0, y: 20 }}
